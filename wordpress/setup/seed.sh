...
set_hours () {
  local company_title="$1" cid
  cid=$(company_id_by_title "$company_title")
  [ -z "$cid" ] && return
  export SH_HID="$cid"
  command wp --allow-root eval 'if(function_exists("update_field")){update_field("company_hours",[["day"=>"mon","open_time"=>"10:00","close_time"=>"20:00"],["day"=>"tue","open_time"=>"10:00","close_time"=>"20:00"],["day"=>"wed","open_time"=>"10:00","close_time"=>"20:00"],["day"=>"thu","open_time"=>"10:00","close_time"=>"20:00"],["day"=>"fri","open_time"=>"10:00","close_time"=>"20:00"],["day"=>"sat","open_time"=>"11:00","close_time"=>"18:00"]],(int)getenv("SH_HID"));}' >/dev/null 2>&1 || true
  echo "  + часы заданы для «$company_title» (#$cid)"
}

echo "→ Часы работы (всем компаниям)…"
for c in "Барбершоп Борода" "Студия Локон" "Дентал Плюс" "Невские Ножницы" "АвтоМастер СПб"; do
  set_hours "$c"
done
